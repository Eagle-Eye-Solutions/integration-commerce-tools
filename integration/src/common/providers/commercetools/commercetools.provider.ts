...
export class Commercetools {
  ...
  public async getExtensionByKey(key: string): Promise<Extension> {
    return (
      await this.getApiRoot().extensions().withKey({ key }).get().execute()
    ).body;
  }
  ...
  public async getSubscriptionByKey(key: string): Promise<Subscription> {
    return (
      await this.getApiRoot().subscriptions().withKey({ key }).get().execute()
    ).body;
  }
